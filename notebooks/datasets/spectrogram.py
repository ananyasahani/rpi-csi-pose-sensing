...
def plot_heatmap(ax, t, values, ylabel, cbar_label, cmap, title):
    n_subcarriers = values.shape[1]
    sc_idx = np.arange(n_subcarriers)
    mesh = ax.pcolormesh(t, sc_idx, values.T, shading="auto", cmap=cmap)
...
def main():
    ...
    plot_heatmap(axes[0], t, amplitude, "Subcarrier index", amp_label, args.cmap,
                 f"CSI Amplitude Spectrogram — {args.pcap.split('/')[-1]}")
